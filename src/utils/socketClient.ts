...
export interface Projectile {
  ...
  exploded?: boolean; // For artillery explosion effects
}
...
export class SocketClient {
  ...
  private onPlayerMoved:
    | ((data: { playerId: string; position: any; rotation: any }) => void)
    | null = null;
  private onProjectileCreated: ((projectile: Projectile) => void) | null = null;
  private onProjectilesUpdate:
    | ((data: { active: Projectile[]; expired: string[] }) => void)
    | null = null;
  private onPlayerHit:
    | ((data: {
        playerId: string;
  ...
  private onPlayerDied:
    | ((data: { playerId: string; killedBy: string }) => void)
    | null = null;
  ...
  setOnPlayerMoved(
    callback: (data: { playerId: string; position: any; rotation: any }) => void
  ) {
  ...
  setOnProjectilesUpdate(
    callback: (data: { active: Projectile[]; expired: string[] }) => void
  ) {
  ...
  setOnPlayerDied(
    callback: (data: { playerId: string; killedBy: string }) => void
  ) {
    this.onPlayerDied = callback;
  }

  setOnPlayerStatsUpdated(
    callback: (data: { playerId: string; health: number; mana: number }) => void
  ) {
    this.onPlayerStatsUpdated = callback;
  }

  setOnPlayerDiedSelf(callback: (data: { message: string }) => void) {
    this.socket?.on("player-died-self", callback);
  }

  setOnPlayerRespawned(
    callback: (data: {
      message: string;
      position: { x: number; y: number; z: number };
      health: number;
  ...
  setOnPlayerDiedOther(
    callback: (data: { playerId: string; username: string }) => void
  ) {
    this.socket?.on("player-died-other", callback);
  }

  setOnPlayerRespawnedOther(
    callback: (data: {
      playerId: string;
      username: string;
      position: { x: number; y: number; z: number };
